        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start md:items-center gap-4">
                ...
                <button
                    ...
                    className="flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-700 text-white px-4 py-2.5 rounded-lg transition-colors shadow-sm w-full sm:w-auto active:scale-95"
                >
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                            <th className="p-1.5 md:p-4 text-[10px] md:text-xs font-semibold text-slate-500 uppercase tracking-tighter md:tracking-normal">{language === 'tr' ? 'Ürün Adı' : 'Product Name'}</th>
                            <th className="p-1.5 md:p-4 text-[10px] md:text-xs font-semibold text-slate-500 uppercase tracking-tighter md:tracking-normal">{language === 'tr' ? 'Kategori' : 'Category'}</th>
                            <th className="p-1.5 md:p-4 text-[10px] md:text-xs font-semibold text-slate-500 uppercase tracking-tighter md:tracking-normal">{language === 'tr' ? 'Miktar' : 'Quantity'}</th>
                            <th className="p-1.5 md:p-4 text-[10px] md:text-xs font-semibold text-slate-500 uppercase tracking-tighter md:tracking-normal">{language === 'tr' ? 'Birim Fiyat' : 'Unit Cost'}</th>
                            <th className="p-1.5 md:p-4 text-[10px] md:text-xs font-semibold text-slate-500 uppercase tracking-tighter md:tracking-normal">{language === 'tr' ? 'SKT' : 'Expiry'}</th>
                            <th className="p-1.5 md:p-4 text-[10px] md:text-xs font-semibold text-slate-500 uppercase text-right tracking-tighter md:tracking-normal">{language === 'tr' ? 'İşlemler' : 'Actions'}</th>
                        </tr>
                    </thead>
                        ...
                        {filteredItems.map(item => (
                            <tr key={item._id} className="hover:bg-slate-50 transition-colors">
                                <td className="p-1.5 md:p-4">
                                    <div className="font-medium text-slate-800 text-xs md:text-base">{item.name}</div>
                                    {item.supplier && <div className="text-[10px] text-slate-400 hidden sm:block">{item.supplier}</div>}
                                </td>
                                <td className="p-1.5 md:p-4">
                                    <span className="px-1.5 py-0.5 md:px-2 md:py-1 bg-slate-100 text-slate-600 rounded text-[10px] md:text-xs whitespace-nowrap">
                                        {getCategoryLabel(item.category)}
                                    </span>
                                </td>
                                <td className="p-1.5 md:p-4">
                                    <div className={`font-semibold text-xs md:text-base ${item.quantity <= item.minLevel ? 'text-red-600' : 'text-slate-700'}`}>
                                        {item.quantity} <span className="text-[10px] md:text-xs font-normal text-slate-500">{item.unit}</span>
                                    </div>
                                </td>
                                <td className="p-1.5 md:p-4 text-slate-600 text-xs md:text-base">
                                    {item.cost > 0 ? `₺${item.cost.toLocaleString()}` : '-'}
                                </td>
                                <td className="p-1.5 md:p-4">
                                    {item.expirationDate ? (
                                        <div className="flex items-center gap-1 text-[10px] md:text-sm text-slate-600">
                                            <Calendar className="w-3 h-3 md:w-3.5 md:h-3.5 text-slate-400 hidden sm:block" />
                                            {new Date(item.expirationDate).toLocaleDateString(undefined, { month: 'numeric', day: 'numeric', year: '2-digit' })}
                                        </div>
                                    ) : '-'}
                                </td>
